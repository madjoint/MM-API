<?php
	...
	*/
	...
	print '<pre>';
	print_r($obj->getCoordinates('Pakistan'));*/
			...
			if ($data){
				...
				if ($requestCode == 200){
					...
					$coords = explode(',',$coords);
				 	if (count($coords) > 1){
				 		if (count($coords) == 3){
						 	return array('lat' => $coords[1], 'long' => $coords[0], 'alt' => $coords[2],'location'=>$location);
						} else {
						 	return array('lat' => $coords[1], 'long' => $coords[0], 'alt' => 0, 'location'=>$location);
						}
					}
				}
			}
			//return default data
			return array('lat' => 0, 'long' => 0, 'alt' => 0,'location'=>0);
		}
