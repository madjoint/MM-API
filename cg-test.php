...
try{
			...
			echo '<br/>Credit Limit = '.$creditlimit.'<br/>';
			echo 'Outstanding = '.$outstanding.'<br/>';
			$bal = $creditlimit - $outstanding.'<br/>';
			echo "Balance :" . $bal;
...
/*try{
$client = new SoapClient("cg1.wsdl",array("trace" =>1, "exceptions" => 0));
$params = array("String_1"=> "EzTrade","arrayOfbyte_2" => "eCtr4d3");
$response = $client->getSessionKey($params);


$client = new SoapClient("cg1.wsdl",array("trace" =>1, "exceptions" => 0));

//print_r($client->ChargedToBillRequest);
$params = new stdClass();
$params->authKey=$response->result;
$params->appID= "EzTrade";
$params->transactionID=time();
$params->domainID ="GSM";
