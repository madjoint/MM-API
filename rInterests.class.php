...
`user` = {$user_id}
		...
		ca_mysql_query("START TRANSACTION;
INSERT INTO `interest_history`  SELECT * FROM `interest` WHERE `id` = {$interest_id} AND  `user` = {$user_id} 
DELETE `interest` WHERE `id` = {$interest_id} AND `user` = {$user_id}
COMMIT;");
...
AND
 ...
 /*******End of Code by kazim raza******/
	public function post_interest($user_id,	$title,	$description, $latitude, $longitude, $distance,	$expire, $image_filename = '',$via) {		
		$prepared_title = preg_replace("/[^a-zA-Z0-9 ]+/", "", $title);
		$lt = rInterests::_interest_prepare($prepared_title); //$title
		...
		$num_less = rInterests::stem_numless_making($lt['stems']);
		
		$iid = ca_mysql_insert("# rInterests::post_interest()
INSERT INTO `interest`
(`user`,`title`,`stems`,`stems_numless`,`kind`,`description`,`latitude`,`longitude`,`distance`,`expire`,`create_time`,`image`, `via`)  
VALUES (
		{$user_id},
'{$title}',
'{$lt['stems']}',
...
__FUNCTION__);
	...
	public function get_interest_with_matches($interest_id) {
		$matched_interests = ca_mysql_query("
SELECT m.`id` AS `match_id`,m.`matched_interest` AS `id`,m.`rank`,m.`unread`,i.`user`,i.`title`,i.`description`,i.`latitude`,i.`longitude`,i.`create_time`,i.`image`,u.`mobile_number`
FROM `match` m, `interest` i, `user` u
WHERE  i.is_live = i.is_live  and 
...
__FUNCTION__);
		$i = rInterests::get_interest($interest_id);
		$match_size = array('matching_count'=>sizeof($matched_interests));
		$result = array_merge($i,$match_size);
		return $result;
		/*if($matched_interests !== False) {
			foreach($matched_interests as &$m) {
				$unread = $m['unread'];
	...
	public function get_list_last($count) {
		$r = ca_mysql_query("
SELECT interest.`id`,interest.`user`,interest.`title`,interest.`description`,interest.`latitude`,interest.`longitude`,interest.`create_time`,interest.`image`,user.mobile_number
FROM `interest` inner join user on user.id = interest.user where interest.is_live = 1 
ORDER BY
interest.`create_time` DESC
LIMIT {$count}
		");
		return($r);
	}	 
	public function get_search_by_page($term,$start=0,$limit=20) {
		...
		$r = ca_mysql_query("
(SELECT SQL_CALC_FOUND_ROWS interest.`id`,interest.`user`,interest.`title`,interest.`description`,interest.`latitude`,interest.`longitude`,interest.`create_time`,interest.`image`,user.mobile_number FROM `interest` inner join user on user.id = interest.user where interest.is_live = 1 AND (
MATCH (title) AGAINST ('%".$term ."%')
)) 
UNION
(SELECT  interest.`id`,interest.`user`,interest.`title`,interest.`description`,interest.`latitude`,interest.`longitude`,interest.`create_time`,interest.`image`,user.mobile_number FROM `interest` inner join user on user.id = interest.user where interest.is_live = 1 AND `title` like('%".$term ."%')) ".$orderby." LIMIT  ".$start ."," .$limit );

//i.`title` like('%".$term ."%')
//ORDER BY interest.`create_time` DESC
	...
	public function get_list($user_id) {
		$r = ca_mysql_query("
SELECT interest.`id`,interest.`user`,interest.`title`,interest.`description`,interest.`latitude`,interest.`longitude`,interest.`create_time`,interest.`image`,user.mobile_number
FROM `interest` inner join user on user.id = interest.user
WHERE 
...
AND
i.`id` = m.`interest` 
AND ( MATCH (title) AGAINST ('%".$keywords ."%'))
...
preg_match_all('/'. $str.'/', $post, $matches);
	...
	function stem_numless_making($stem)
	{
	    if(is_array($stem))
	    {
		$stem = implode(' ',$stem);
	    ...
	    //$txt = "sell apple iphone4 year2011 lg-kg220 /fff .df";
	    $clean_txt= preg_replace('/[0-9\.-\/][a-zA-Z]*/','',$stem);
	    $stem = explode(' ',$stem);
	    $clean_txt = explode(' ',$clean_txt);
	    
	    //remove empty array index
	    for($i=0;$i<sizeof($clean_txt);$i++)
	    {
		if($clean_txt[$i]=='' or $clean_txt[$i]== NULL )
		{
			unset($clean_txt[$i]);  
		}
	    }
	    
	    $diff = array_diff($stem,$clean_txt);
	    //print_r($c);
	    $stem_numless = array_merge($clean_txt,$diff);
	    
	    for($i=0;$i<sizeof($stem_numless);$i++)
	    {
		if(is_numeric($stem_numless[$i]))
		{
		    unset($stem_numless[$i]);  
		}
	    }

	    $stem_numless = implode(' ',$stem_numless);
	    return $stem_numless;
	}
}
?>
